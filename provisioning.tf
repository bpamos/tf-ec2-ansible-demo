...
resource "null_resource" "remote-config" {
  ...
  depends_on = [aws_instance.nginx, aws_eip_association.eip-assoc, null_resource.inventory-setup, null_resource.ssh-setup]
...
data "template_file" "ansible_inventory" {
  ...
  vars = {
    host_ip  = element(aws_eip.nginx_eip.*.public_ip, count.index)
    vpc_name = format("%s-%s-cluster-vpc", var.base_name, var.region)
    ncount   = count.index
  }
  depends_on = [aws_instance.nginx, aws_eip_association.eip-assoc, aws_volume_attachment.ephemeral_re_cluster_instance]
}
#aws_volume_attachment.ephemeral_re_cluster_instance
...
resource "null_resource" "ansible-run" {
  ...
  provisioner "local-exec" {
    ...
    command = "ansible-playbook ${path.module}/ansible/nginx.yaml --private-key ${local.private_key_path} -i /tmp/${format("%s-%s-cluster-vpc", var.base_name, var.region)}_node_${count.index}.ini --become -e 'ENABLE_VOLUMES=${var.enable-volumes}'"
    }
